...
export class DepartmentSyncService {
  ...
  private async fetchPage(
    ...
    const res = await fetch(`${this.baseUrl()}/department/get`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      throw new Error(
        `Departments fetch failed (page ${page}): ${res.status} ${await res.text()}`,
      );
    }
    return (await res.json()) as OracleDepartmentsResponse;
  }
