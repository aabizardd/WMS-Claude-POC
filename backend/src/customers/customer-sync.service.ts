...
export class CustomerSyncService {
  ...
  private async fetchPage(
    ...
    const res = await fetch(`${this.baseUrl()}/customers/get`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      throw new Error(
        `Customers fetch failed (page ${page}): ${res.status} ${await res.text()}`,
      );
    }
    return (await res.json()) as OracleCustomersResponse;
  }
