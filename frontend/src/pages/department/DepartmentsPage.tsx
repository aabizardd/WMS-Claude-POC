...
import type { Department, ErpSyncResult, Paginated } from '../../types';
...
export default function DepartmentsPage() {
  const { has } = useAuth();
  const canSync = has('departments:sync');
  ...
  async function load() {
    setLoading(true);
    const r = await api.get<Paginated<Department>>('/departments', {
  ...
  async function handleSync(e: FormEvent) {
    ...
    try {
      ...
      const r = await api.post<ErpSyncResult>('/departments/sync-erp', payload);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {data ? `${data.total_data} departments` : 'Departments'} · synced from
            Oracle (read-only).
          </p>
        </div>
        {canSync && (
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <SortableTh label="Parent" col="parent_name" sort={sort} onSort={onSort} />
                <SortableTh label="Subsidiary" col="subsidiary_name" sort={sort} onSort={onSort} />
                <SortableTh label="Status" col="is_inactive" sort={sort} onSort={onSort} />
              ...
              {loading ? (
                ...
                data.rows.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50">
                    <td className="px-6 py-3 text-slate-500">{d.oracle_id}</td>
                    <td className="px-6 py-3 font-medium text-slate-800">
                      {d.name ?? '—'}
                    </td>
                    <td className="px-6 py-3 text-slate-600">
                      {d.parent_name || <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-6 py-3 text-slate-600">
                      {d.subsidiary_name || (
                    ...
                    <td className="px-6 py-3">
                      ...
                      >
                        {d.is_inactive ? 'Inactive' : 'Active'}
                      </span>
                    </td>
                    <td className="px-6 py-3 text-slate-600">
                      {d.last_modified
                        ? new Date(d.last_modified).toLocaleDateString()
                        : '—'}
                    </td>
                  </tr>
                ))
      ...
      <Modal
        open={syncOpen}
        title="Sync departments from ERP"
      ...
      >
        <form onSubmit={handleSync} className="space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Pull departments from Oracle and upsert them. The sync window is
