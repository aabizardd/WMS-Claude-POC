...
import type { ClassMaster, ErpSyncResult, Paginated } from '../../types';
...
export default function ClassesPage() {
  const { has } = useAuth();
  const canSync = has('classes:sync');

  const [data, setData] = useState<Paginated<ClassMaster> | null>(null);
  ...
  async function load() {
    setLoading(true);
    const r = await api.get<Paginated<ClassMaster>>('/classes', {
  ...
  async function handleSync(e: FormEvent) {
    ...
    try {
      ...
      const r = await api.post<ErpSyncResult>('/classes/sync-erp', payload);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {data ? `${data.total_data} classes` : 'Classes'} · synced from
            Oracle (read-only).
          </p>
        </div>
        {canSync && (
                ...
                data.rows.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-3 text-slate-600">
                      {c.parent_name || <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-6 py-3 text-slate-600">
                      {c.subsidiary_name || (
      ...
      >
        <form onSubmit={handleSync} className="space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Pull classes from Oracle and upsert them. The sync window is
