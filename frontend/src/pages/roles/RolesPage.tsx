...
export default function RolesPage() {
  ...
  async function load() {
    setLoading(true);
    const r = await api.get<Paginated<Role>>('/roles', {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {data ? `${data.total_data} roles` : 'Roles'} · assigned permissions
            (menu access + CRUD + sync).
          </p>
        </div>
        {canCreate && (
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <SortableTh label="Permissions" col="permissions" sort={sort} onSort={onSort} />
                <SortableTh label="Users" col="users" sort={sort} onSort={onSort} />
