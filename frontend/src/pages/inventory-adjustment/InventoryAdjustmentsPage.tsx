...
export default function InventoryAdjustmentsPage() {
  ...
  const [data, setData] = useState<Paginated<InventoryAdjustmentRow> | null>(
    null,
  );
  ...
  async function load() {
    setLoading(true);
    const r = await api.get<Paginated<InventoryAdjustmentRow>>(
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <SortableTh
                  label="Oracle Approval Status"
                  col="oracle_approval_status"
                  sort={sort}
                  onSort={onSort}
                />
              ...
              {loading ? (
                ...
                data.rows.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-3">
                      <span className={`badge ${oracleStatusBadge(a.oracle_approval_status ?? "-")}`}>
                        {a.oracle_approval_status ?? "—"}
                      </span>
                    </td>
