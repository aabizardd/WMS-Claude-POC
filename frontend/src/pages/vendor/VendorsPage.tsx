...
import type { ErpSyncResult, Paginated, Vendor } from '../../types';
...
export default function VendorsPage() {
  const { has } = useAuth();
  const canSync = has('vendors:sync');

  const [data, setData] = useState<Paginated<Vendor> | null>(null);
  ...
  async function load() {
    setLoading(true);
    const r = await api.get<Paginated<Vendor>>('/vendors', {
  ...
  async function handleSync(e: FormEvent) {
    ...
    try {
      ...
      const r = await api.post<ErpSyncResult>('/vendors/sync-erp', payload);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {data ? `${data.total_data} vendors` : 'Vendors'} · synced from Oracle
            (read-only).
          </p>
        </div>
        {canSync && (
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <SortableTh label="Subsidiary" col="subsidiary_display" sort={sort} onSort={onSort} />
              ...
              {loading ? (
                ...
                data.rows.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-50">
                    <td className="px-6 py-3 text-slate-500">{v.oracle_id}</td>
                    ...
                    <td className="px-6 py-3 text-slate-600">
                      {v.subsidiary_display ?? '—'}
                    </td>
                    <td className="px-6 py-3 text-slate-600">
                      {v.last_modified
                        ? new Date(v.last_modified).toLocaleDateString()
                        : '—'}
                    </td>
                  </tr>
                ))
      ...
      >
        <form onSubmit={handleSync} className="space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Pull vendors from Oracle and upsert them. The sync window is
