...
import type { ErpSyncResult, Paginated, Subsidiary } from '../../types';
...
export default function SubsidiariesPage() {
  const { has } = useAuth();
  const canSync = has('subsidiaries:sync');

  const [data, setData] = useState<Paginated<Subsidiary> | null>(null);
  ...
  async function load() {
    setLoading(true);
    const r = await api.get<Paginated<Subsidiary>>('/subsidiaries', {
  ...
  async function handleSync(e: FormEvent) {
    ...
    try {
      ...
      const r = await api.post<ErpSyncResult>('/subsidiaries/sync-erp', payload);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-slate-800">Subsidiaries</h1>
          <p className="mt-1 text-sm text-slate-500">
            {data ? `${data.total_data} subsidiaries` : 'Subsidiaries'} · synced
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <SortableTh label="Full Name" col="full_name" sort={sort} onSort={onSort} />
                <SortableTh label="Status" col="is_delete" sort={sort} onSort={onSort} />
              ...
              {loading ? (
                ...
                data.rows.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50">
                    <td className="px-6 py-3 text-slate-500">{s.oracle_id}</td>
                    <td className="px-6 py-3 font-medium text-slate-800">
                      {s.name ?? '—'}
                    </td>
                    <td className="px-6 py-3 text-slate-600">
                      {s.full_name || <span className="text-slate-300">—</span>}
                    </td>
      ...
      <Modal
        open={syncOpen}
        title="Sync subsidiaries from ERP"
      ...
      >
        <form onSubmit={handleSync} className="space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Pull subsidiaries from Oracle and upsert them. The sync window is
